...
function game_update(dt)
	--[ hacky mouse position code to make a "borderless" game [doesnt work :()]
	local mx,my, change = love.mouse.getPosition()
	if mx <= 0 then
		mx = screenwidth-2
		change = true
	elseif mx >= screenwidth-1 then
		mx = 1
		change = true
	end
	if my <= 0 then
		my = screenheight-2
		change = true
	elseif my >= screenheight-1 then
		my = 1
		change = true
	end
	if change then -- because we all hate change
		love.mouse.setPosition(mx, my)
		change = nil
	end

	if (mousex~=mx) or (mousey~=my) then
		if (not smoothfound) or (math.abs(mousex-mx) > 4) or (math.abs(mousey-my) > 4) then
			mousex, mousey = mx, my
	...
	if fadetimer3 ~= fadetimer3t then
		...
		if fadetimer3 >= fadetimer3t then
			...
			if rotatedrag then
				rotatedragX, rotatedragY = mousex, mousey
			end
		end
	end
	...
	-- mouse rotation
	if ( (os.clock()-lastMouseUpdate) < 0.1 ) and (not gamepaused) then
		if not rotatedrag then
			print("Rotating")
			rotatedrag = true
		end
		updateperspective("none")
	...
	elseif rotatedrag then -- snap to grid
		if rotatedrag then
...
function updateperspective(persp)
	...
	if lastperspective == persp then return end
...
function game_keypressed(key)
	...
	if key == "escape" and fadetimer3 == 1.5 and not won then
